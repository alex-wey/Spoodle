...
const inputClass =
  "h-11 rounded-xl border-white/10 bg-white/[0.06] text-[15px] text-foreground shadow-inner placeholder:text-white/35 " +
  "focus-visible:border-[#4559A7]/60 focus-visible:ring-2 focus-visible:ring-[#4559A7]/30 transition-[border-color,box-shadow]";

const selectTriggerClass =
  "h-11 rounded-xl border-white/10 bg-white/[0.06] text-[15px] shadow-inner " +
  "focus:ring-2 focus:ring-[#4559A7]/30 data-[placeholder]:text-white/35";
...
export default function VerumAccessRequest({ onGranted }: { onGranted: () => void }) {
  ...
  const eduValid = useMemo(() => isEduEmail(stuEmail), [stuEmail]);
  const studentBypassReady = useMemo(() => isBypassEmail(stuEmail), [stuEmail]);
  const studentCanSubmit = eduValid || studentBypassReady;
  ...
  const handleStudentSubmit = (e: React.FormEvent) => {
    ...
    if (!isEduEmail(stuEmail)) {
      setStudentEmailError("Please use a valid school email address ending in .edu");
      return;
    }
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
                  ...
                  >
                    ...
                    <span className="mt-1 text-xs leading-snug text-white/50">
                      AVMA-accredited program — verify with a .edu email
                    </span>
                  </button>
                </div>
                <Button
              ...
              <form onSubmit={handleStudentSubmit} className="space-y-5">
                <div className="flex items-start justify-between gap-3 border-b border-white/10 pb-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-white/45">Use your school-issued .edu address.</p>
                  </div>
                  <Button
                    type="button"
                    variant="ghost"
              ...
              <div className="space-y-6 text-center">
                ...
                <div>
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-white/55">
                    Your school email checks out. Continue to open Spoodle with the profile you provided.
                  </p>
                </div>
                <Button
                  type="button"
