  ...
  try {
    ...
    if (!apiKey || apiKey === "sk-proj-your-openai-api-key-here") {
      ...
      return NextResponse.json({
        success: true,
        data: fallback,
        message: "Response generated (fallback - set OPENAI_API_KEY in .env.local for AI responses)",
      });
    }
...
7. Be professional and cite appropriately`;
    ...
    const result = JSON.parse(jsonStr) as { content?: string; sources?: VerumSource[] };
