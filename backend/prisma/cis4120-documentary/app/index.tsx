...
function isAudioFile(file: { mimeType?: string; name?: string } | null): boolean {
  if (!file) return false;
  const mime = (file.mimeType || '').toLowerCase();
  if (mime.startsWith('audio/')) return true;
  const n = (file.name || '').toLowerCase();
  return /\.(mp3|m4a|aac|wav|ogg|flac|opus|aiff?|caf)$/i.test(n);
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function syncSound() {
      ...
      if (screen !== 'moments' || !selectedFile || !isAudioFile(selectedFile)) return;
      ...
      try {
        ...
        /* load failed — controls stay disabled until user reopens clip */
      }
    }
  ...
  const handleSeek = async () => {
    ...
    if (selectedFile && isAudioFile(selectedFile)) {
      if (soundRef.current) await soundRef.current.setPositionAsync(timeMs);
      return;
    }
    if (videoRef.current) await videoRef.current.setPositionAsync(timeMs);
  };
              ...
              <>
                {isAudioFile(selectedFile) ? (
                  <View style={s.audioPanel}>
                    <MaterialCommunityIcons name="music-note" size={48} color="#6695FC" />
                    <Text style={s.audioPanelLabel}>Audio clip</Text>
                    <Text style={s.audioPanelName} numberOfLines={2}>{selectedFile.name}</Text>
                  </View>
                ) : (
                  <Video
                    ref={videoRef}
...
const s = StyleSheet.create({
  ...
  audioPanelLabel: { fontSize: 12, fontWeight: '600', color: '#9CA3AF', marginTop: 8 },
  audioPanelName: { fontSize: 13, fontWeight: '500', color: '#E5E7EB', marginTop: 4, textAlign: 'center' },
